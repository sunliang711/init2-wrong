...
install(){
    if [ ! -d ${home}/.cargo ];then
        echo "No ${home}/.cargo,exit."
        return 1
    fi
    local configFile=${home}/.cargo/config
    cat<<EOF>${configFile}
${begin}
[source.crates-io]
replace-with = 'tuna'
...
uninstall(){
    ...
    if [ ! -e ${configFile} ];then
        echo "no ${configFile}, exit."
        return 1
    fi

    perl -lne "print unless /${begin}/../${end}/" ${configFile} > ${configFile}.tmp && mv ${configFile}.tmp ${configFile}
